...
/// Remembers the type ('song' / 'video') detected while parsing a search
/// result. Used to group mixed search results without depending on the
/// text label YouTube puts in front of the subtitle.
final Expando<String> searchResultTypeOf = Expando<String>();
...
List<dynamic> parseSearchResults(List<dynamic> results,
  ...
  for (final result in results) {
    // A shelf can also contain entries that are not songs/albums/etc
    // (e.g. `messageRenderer`, `continuationItemRenderer`). Skip them
    // instead of crashing the whole search.
    final renderer =
        result is Map ? result['musicResponsiveListItemRenderer'] : null;
    if (renderer == null) continue;
    try {
      final item = parseSearchResult(
          renderer, searchResultTypes, resultType, category);
      // drop unparsable / unavailable entries (null or empty map)
      if (item == null || (item is Map && item.isEmpty)) continue;
      parsed.add(item);
    } catch (e) {
      // one bad item should not hide all the other results
      printERROR("Skipped unparsable search item: $e");
    }
  }
  return parsed;
}
...
dynamic parseSearchResult(Map<String, dynamic> data,
  ...
  if (resultType == 'song' || resultType == 'video') {
    if (searchResult['videoId'] != null) {
      final item = MediaItemBuilder.fromJson(searchResult);
      searchResultTypeOf[item] = resultType;
      return item;
    }
    return;
  } else if (resultType.contains('playlist')) {
...
dynamic parseSearchResult(Map<String, dynamic> data,
  ...
  if ((['song', 'video', 'album']).contains(resultType)) {
    ...
    searchResult['year'] = null;
    final flexItem = getFlexColumnItem(data, 1);
    // getFlexColumnItem returns {} (no 'text' key) when YouTube's response
    // shape doesn't have this column -- reading ['text']['runs'] straight
    // off that used to throw and silently drop the whole song/video/album
    // from the results (caught by parseSearchResults' try/catch).
