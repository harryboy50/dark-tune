...
class MusicServices extends getx.GetxService {
  ...
  Future<Map<String, dynamic>> search(String query,
    ...
    for (var res in results) {
      ...
      if (res['musicShelfRenderer'] != null) {
        ...
        // shelf without any real result (e.g. only a `messageRenderer`)
        final hasResultItems = (itemResults as List).any((e) =>
            e is Map && e['musicResponsiveListItemRenderer'] != null);
        if (!hasResultItems) {
        ...
        if (filter == null) {
          for (var item in mixedItems) {
            final itemType = _searchItemCategory(item);
          ...
          // Don't depend on the shelf title (it can be missing/changed by
          // YouTube). The key is derived from the selected filter and matches
          // the tab names used by the UI ("Songs", "Featured playlists"...).
  ...
  /// Category key ("Songs", "Videos", "Albums" ...) of an item found in the
  /// mixed (unfiltered) search results.
  String _searchItemCategory(dynamic item) {
    if (item is MediaItem) {
      final type = searchResultTypeOf[item];
      if (type == 'song') return "Songs";
      if (type == 'video') return "Videos";
      // fallback: old behaviour (type label is the first "artist" name)
      return "${(item.artist ?? '').split(",")[0]}s";
    }
    return "${item.runtimeType}s";
  }

  String _categoryFromFilter(String? filter) {
    if (filter == null || filter.isEmpty) return "mixed";
    final text = filter.replaceAll("_", " ");
